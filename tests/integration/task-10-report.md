...
集成检查使用当前 v2 契约：公共请求走 `/api/v2`，Java 到 Python 的任务和回调走
`/internal/v2`。业务 ID 使用 `user001`、`resume001`、`task001` 等可读格式；仅用于
链路追踪的 `correlationId` 仍按契约使用独立的追踪格式。
...
- 简历页面缓存使用 `resume:v2:view:<resumeId>` 键；删除和归档只驱逐该派生缓存，MySQL 记录仍保留。
