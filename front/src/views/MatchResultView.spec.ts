...
const { lifecycleApi, routeParams } = vi.hoisted(() => ({ lifecycleApi: { getMatchTask: vi.fn(), getMatchResult: vi.fn() }, routeParams: { taskId: 'task001', proxy: null as { taskId: string } | null } }))
...
describe('MatchResultView task lifecycle', () => {
  beforeEach(() => { vi.useFakeTimers(); if (routeParams.proxy) routeParams.proxy.taskId = 'task001'; lifecycleApi.getMatchTask.mockReset(); lifecycleApi.getMatchResult.mockReset() })
