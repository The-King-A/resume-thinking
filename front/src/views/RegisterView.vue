...
async function submit() {
  ...
  else if (form.password !== form.confirmPassword) validationMessage.value = '两次输入的密码不一致'
  if (form.username.length === 0) validationMessage.value = '用户名不能为空'
  if (validationMessage.value) return
  try { await auth.register({ username: form.username, email: form.email, password: form.password, role: form.role }); await router.push('/profiles') } catch { /* store exposes safe message */ }
}
</script>
<template>
  <main class="auth-page"><section class="auth-panel"><p class="eyebrow">简历匹配平台</p><h1>创建账号</h1><p class="muted">设置你的工作区和模型访问权限。</p>
    <el-form ref="formRef" :model="form" :rules="rules" @submit.prevent="submit">
      ...
      <el-form-item label="确认密码" prop="confirmPassword"><input v-model="form.confirmPassword" type="password" autocomplete="new-password-confirmation" /></el-form-item>
      <fieldset><legend>账号角色</legend><label><input type="radio" name="role" value="USER" v-model="form.role" data-test="role-user" /> 普通用户</label><label><input type="radio" name="role" value="ADMIN" v-model="form.role" data-test="role-admin" /> 管理员</label><p class="notice">管理员可以处理跨用户的恢复记录。这是全局恢复权限，并非仅用于演示；请仅在确实需要该权限时选择。</p></fieldset>
      <p v-if="validationMessage" class="error">{{ validationMessage }}</p><button data-test="register-submit" type="submit" @click.prevent="submit" :disabled="auth.loading">创建账号</button>
