...
const active = { id: 'resume001', ownerId: 'user001', title: 'resume', sourceType: 'TXT', status: 0, visibilityState: 'ACTIVE', version: 2, visibleUntil: null, softDeletedAt: null, archivedAt: null, restoredAt: null, createdAt: '', updatedAt: '' }
...
describe('ResumeListView role-scoped deletion', () => {
  ...
  it('uses the user delete route for a USER', async () => {
    ...
    expect(lifecycleApi.deleteResume).toHaveBeenCalledWith('resume001', { confirmationText: '确认删除简历', expectedVersion: 2 }); expect(lifecycleApi.adminSoftDeleteResume).not.toHaveBeenCalled()
  })
  ...
  it('uses the administrator soft-delete route for an ADMIN', async () => {
    ...
    await wrapper.get('.button-danger-quiet').trigger('click'); await wrapper.get('input').setValue('确认删除简历'); await wrapper.get('[data-test="confirm-delete"]').trigger('click'); await flushPromises()
    expect(lifecycleApi.adminSoftDeleteResume).toHaveBeenCalledWith('resume001', { confirmationText: '确认删除简历', expectedVersion: 2 }); expect(lifecycleApi.deleteResume).not.toHaveBeenCalled()
  })
})
