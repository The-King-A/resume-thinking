...
from .interview_service import analyze_interview_job
from .callback_client import CallbackClient
from .models import AnalysisJob, V3AnalysisJob
from .interview_models import InterviewJob
...
async def require_internal_service_auth(
    ...
    # Weak or malformed configuration fails closed. compare_digest avoids
    # timing-based token checks once an operator has configured the secret.
    if (
        not is_usable_internal_service_token(expected)
        or not token
...
async def submit_analysis_job(
    ...
    return await _submit_analysis_job(job, background_tasks)


@app.post("/internal/v3/analysis-jobs", status_code=202)
async def submit_v3_analysis_job(
    job: V3AnalysisJob,
    background_tasks: BackgroundTasks,
    _auth: Annotated[None, Depends(require_internal_service_auth)],
) -> dict[str, str]:
...
@app.post("/internal/v4/interview-jobs", status_code=202)
async def submit_interview_job(
    job: InterviewJob,
    background_tasks: BackgroundTasks,
    _auth: Annotated[None, Depends(require_internal_service_auth)],
) -> dict[str, str]:
    ...
    background_tasks.add_task(process)
    return {"status": "accepted", "sessionId": str(job.session_id)}
