...
class InternalAnalysisCallbackControllerTest {
    ...
    void callbackAfterArchiveReturnsTaskGoneAndDoesNotPersistResult() {
        String owner = TestIds.user();
        String resumeId = TestIds.resume();
        ...
        var task = service.createTask(new CreateMatchTaskCommand(owner, resumeId, TestIds.profile(),
        ...
        var response = new InternalAnalysisCallbackController(service).accept(new AnalysisCallbackRequest(
                task.id(), 1, task.callbackId(), task.callbackTokenForTests(), "",
    ...
    void invalidTokenCannotBlockAnInFlightTaskEvenWhenResumeIsAlreadyInactive() {
        ...
        var response = service.acceptCallback(new AnalysisCallbackRequest(
                task.id(), 1, task.callbackId(), "x".repeat(32), "", "FAILED", null,
    ...
    void duplicateCallbackIsAcceptedReplayAndChangedPayloadConflicts() {
        String owner = TestIds.user(); String resumeId = TestIds.resume();
        ResumeRepository resumes = new ResumeRepository.InMemory();
        ...
        String callbackId = task.callbackId();
    ...
    void staleAttemptIsRejectedWithoutWritingResult() {
        ...
        var stale = new AnalysisCallbackRequest(task.id(), 0, task.callbackId(), task.callbackTokenForTests(), "", "FAILED", null, "MODEL_UNAVAILABLE", UUID.randomUUID()).withComputedPayloadHash();
    ...
    void callbackIdCannotBeReplayedForAnotherTaskOrWithAnotherToken() {
        String owner = TestIds.user(); String resumeOne = TestIds.resume(); String resumeTwo = TestIds.resume();
        ResumeRepository resumes = new ResumeRepository.InMemory();
        ...
        var firstTask = service.createTask(new CreateMatchTaskCommand(owner, resumeOne, TestIds.profile(),
                "Build reliable software with clear communication and practical testing.", "cross-task-key-001"));
        var secondTask = service.createTask(new CreateMatchTaskCommand(owner, resumeTwo, TestIds.profile(),
                "Build reliable software with clear communication and practical testing.", "cross-task-key-002"));
        String callbackId = firstTask.callbackId();
    ...
    private static final class SwitchingTaskRepository implements MatchTaskRepository {
        ...
        @Override public synchronized Optional<MatchTask> findById(String id) { return delegate.findById(id); }
        @Override public synchronized Optional<MatchTask> findByCreatorIdAndIdempotencyKey(String owner, String key) {
            return delegate.findByCreatorIdAndIdempotencyKey(owner, key);
        }
        @Override public synchronized Optional<MatchTask> findByIdForUpdate(String id) { return delegate.findByIdForUpdate(id); }
        @Override public synchronized List<MatchTask> findByResumeIdAndStateInForUpdate(String resumeId, Collection<MatchTask.State> states) {
            return delegate.findByResumeIdAndStateInForUpdate(resumeId, states);
        }
        @Override
        public synchronized Optional<MatchTask> lockById(String id) {
            lockCalls++;
    ...
    private static final class ArchivingRaceReceipts implements CallbackReceiptRepository {
        ...
        @Override
        public synchronized Optional<CallbackReceipt> findByCallbackId(String callbackId) {
