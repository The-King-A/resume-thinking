...
public final class InMemoryRepositories {
 static class UserRepositoryStub implements UserRepository {
  ...
  public Optional<User> findById(String id){return Optional.ofNullable(values.get(id));}
 ...
 public static class LlmProfileRepositoryStub implements LlmProfileRepository {
  private final Map<String,LlmProfile> values=new HashMap<>();
  public LlmProfile save(LlmProfile p){values.put(p.getId(),p);return p;}
  public Optional<LlmProfile> findByIdAndOwnerId(String id,String owner){return Optional.ofNullable(values.get(id)).filter(p->p.getOwnerId().equals(owner));}
  public List<LlmProfile> findAllByOwnerId(String owner){return values.values().stream().filter(p->p.getOwnerId().equals(owner)).toList();}
