...
-- V1-V7 analysis_tasks has no callback_id column; task callback IDs are allocated
-- after the existing receipt IDs so both callback sources remain globally unique.
...
CREATE TEMPORARY TABLE task_callback_id_map (old_id BINARY(16) PRIMARY KEY, new_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL UNIQUE);
INSERT INTO task_callback_id_map
    SELECT id, CONCAT('callback', LPAD((SELECT COUNT(*) FROM analysis_callback_receipts) + ROW_NUMBER() OVER (ORDER BY created_at, id), 3, '0'))
    FROM analysis_tasks;
...
INSERT INTO v8_mapping_check SELECT COUNT(*) FROM resumes r LEFT JOIN user_id_map u ON u.old_id = r.soft_deleted_by WHERE r.soft_deleted_by IS NOT NULL AND u.new_id IS NULL;
...
INSERT INTO v8_mapping_check SELECT COUNT(*) FROM analysis_tasks t LEFT JOIN task_callback_id_map c ON c.old_id = t.id WHERE c.new_id IS NULL;
-- V1-V7 did not persist task callback IDs. Do not convert while an old worker
-- could still send a callback whose UUID cannot be associated after V8.
INSERT INTO v8_mapping_check SELECT COUNT(*) FROM analysis_tasks WHERE state IN ('QUEUED', 'PROCESSING');
...
-- A maintenance view created against the old UUID columns can block column
-- replacement and would return unreadable values after the migration.
...
ALTER TABLE analysis_tasks ADD COLUMN id_v2 VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL, ADD COLUMN callback_id_v2 VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL, ADD COLUMN resume_id_v2 VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL, ADD COLUMN llm_profile_id_v2 VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL, ADD COLUMN creator_id_v2 VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL;
...
UPDATE analysis_tasks t JOIN task_callback_id_map m ON m.old_id = t.id SET t.callback_id_v2 = m.new_id;
...
ALTER TABLE analysis_tasks DROP PRIMARY KEY, DROP COLUMN id, DROP COLUMN resume_id, DROP COLUMN llm_profile_id, DROP COLUMN creator_id, CHANGE COLUMN id_v2 id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL PRIMARY KEY COMMENT '分析任务唯一标识', CHANGE COLUMN callback_id_v2 callback_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT 'Java签发的回调消息标识', CHANGE COLUMN resume_id_v2 resume_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '关联简历标识', CHANGE COLUMN llm_profile_id_v2 llm_profile_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '使用的模型配置标识', CHANGE COLUMN creator_id_v2 creator_id VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '任务创建者标识';
...
ALTER TABLE analysis_tasks ADD CONSTRAINT uq_analysis_task_callback_id UNIQUE (callback_id), ADD CONSTRAINT uq_analysis_task_owner_key UNIQUE (creator_id, idempotency_key), ADD CONSTRAINT fk_analysis_task_resume FOREIGN KEY (resume_id) REFERENCES resumes(id), ADD CONSTRAINT fk_analysis_task_profile FOREIGN KEY (llm_profile_id) REFERENCES llm_profiles(id), ADD CONSTRAINT fk_analysis_task_creator FOREIGN KEY (creator_id) REFERENCES users(id);
...
-- Recreate the optional maintenance view with v2 string IDs. It exposes only
-- metadata and ciphertext size; resume content and encryption material remain
-- outside the view.
CREATE OR REPLACE VIEW v_resumes_readable AS
SELECT id, owner_id, title, source_type, parser_version,
       OCTET_LENGTH(raw_content_ciphertext) AS encrypted_content_bytes,
       HEX(raw_content_nonce) AS raw_content_nonce_hex,
       creator_role, status, visibility_state, visible_until,
       soft_deleted_by, soft_deleted_at, archived_at, restored_at,
...
UNION ALL SELECT 'callback', GREATEST(
...
) + 1 FROM analysis_callback_receipts
...
DROP TEMPORARY TABLE user_id_map, profile_id_map, resume_id_map, task_id_map, evidence_id_map, result_id_map, callback_id_map, task_callback_id_map, audit_id_map;
