...
public class AesGcmCryptoService {
    ...
    public AesGcmCryptoService(@Value("${app.encryption-key-base64:}") String encoded) {
        if (encoded == null || encoded.isBlank()) throw new IllegalStateException("app.encryption-key-base64 must be configured");
        byte[] bytes; try { bytes = Base64.getDecoder().decode(encoded); } catch (Exception e) { throw new IllegalStateException("app.encryption-key-base64 must be valid base64", e); }
        if (bytes.length != 32) throw new IllegalStateException("app.encryption-key-base64 must decode to 256 bits"); key = new SecretKeySpec(bytes, "AES");
    }
