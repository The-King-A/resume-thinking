...
public class ResumeController {
 ...
 @GetMapping("/resumes") public Page list(@RequestAttribute("actorId") UUID actorId,@RequestAttribute("role") UserRole role,@RequestParam(defaultValue="1") int page,@RequestParam(defaultValue="20") int pageSize){int p=normalizedPage(page),s=normalizedSize(pageSize); return Page.from(service.listActive(actorId,role,PageRequest.of(p-1,s,Sort.by("id"))),p,s);}
 ...
 private static int normalizedPage(int page){return Math.max(1,page);} private static int normalizedSize(int size){return Math.max(1,Math.min(100,size));}
 public record Page(List<Response> items,int page,int pageSize,long totalItems,int totalPages){static Page from(org.springframework.data.domain.Page<Resume> data,int page,int size){return new Page(data.getContent().stream().map(Response::from).toList(),page,size,data.getTotalElements(),data.getTotalPages());}}
