...
public class Resume {
    ...
    public static Resume active(UUID id, UUID ownerId, String title, SourceType sourceType, UserRole creatorRole, Instant createdAt, long version) {
        ...
        r.encryptedRawContent=new byte[0]; r.status=0; r.visibilityState=VisibilityState.ACTIVE; r.createdAt=createdAt; r.updatedAt=createdAt;
    ...
    public Resume(UUID ownerId, String title, SourceType sourceType, UserRole creatorRole, byte[] encryptedRawContent, Instant now) {
        this(UUID.randomUUID(), ownerId, title, sourceType, creatorRole, encryptedRawContent, now);
    }
    public Resume(UUID id, UUID ownerId, String title, SourceType sourceType, UserRole creatorRole, byte[] encryptedRawContent, Instant now) {
        this.id=id; this.ownerId=ownerId; this.title=title; this.sourceType=sourceType; this.creatorRole=creatorRole; this.encryptedRawContent=encryptedRawContent;
    ...
    public void softDelete(UUID actorId, UserRole role, Instant now) { status=1; visibilityState=role==UserRole.ADMIN?VisibilityState.ADMIN_SOFT_DELETED:VisibilityState.USER_SOFT_DELETED; softDeletedBy=actorId; softDeletedAt=now; updatedAt=now; version++; }
    public void archive(Instant now) { if (visibilityState==VisibilityState.ACTIVE) { visibilityState=creatorRole==UserRole.ADMIN?VisibilityState.ADMIN_CACHE_ARCHIVED:VisibilityState.USER_CACHE_ARCHIVED; archivedAt=now; updatedAt=now; version++; } }
    public void restore(Instant now) { status=0; visibilityState=VisibilityState.ACTIVE; softDeletedBy=null; softDeletedAt=null; archivedAt=null; restoredAt=now; updatedAt=now; visibleUntil=now.plus(creatorRole==UserRole.ADMIN?Duration.ofDays(30):Duration.ofDays(7)); version++; }
    public UUID getId(){return id;} public UUID id(){return id;} public UUID getOwnerId(){return ownerId;} public String getTitle(){return title;} public SourceType getSourceType(){return sourceType;} public byte[] getEncryptedRawContent(){return encryptedRawContent;} public UserRole getCreatorRole(){return creatorRole;} public int getStatus(){return status;} public VisibilityState getVisibilityState(){return visibilityState;} public Instant getVisibleUntil(){return visibleUntil;} public UUID getSoftDeletedBy(){return softDeletedBy;} public Instant getSoftDeletedAt(){return softDeletedAt;} public Instant getArchivedAt(){return archivedAt;} public Instant getRestoredAt(){return restoredAt;} public Instant getCreatedAt(){return createdAt;} public Instant getUpdatedAt(){return updatedAt;} public long getVersion(){return version;}
}
